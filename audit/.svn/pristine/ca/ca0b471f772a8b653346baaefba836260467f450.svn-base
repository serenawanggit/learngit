...
Dialog.prototype = {
	// 参数设置
	getOptions : function( arg,clasName ){
		var i,
			options = {},
			// 默认参数
			defaults = {
	...
	createOverlay : function(){
		...
		style.cssText = 'margin:0;padding:0;border:none;width:100%;height:100%;background:#77887E;opacity:0.6;filter:alpha(opacity=60);z-index:1000;position:absolute;top:0;left:0;';
	...
	createDialogBox : function(){
		...
		dialogBox.style.cssText = 'padding:0;border:none;z-index:10000;';
	...
	createDialogWrap : function( tmpl ){
		...
		var header = tmpl.header ? 
			'<h4 class="easyDialog_title" id="easyDialogTitle" style="height:35px;background-image:url(../images/showtitlebk800.png);">'+
				'<a href="javascript:void(0)" title="关闭窗口" class="close_btn" id="closeBtn"><img src="../images/alertimg/alertclose.png" border="0"></a>' + 
			'<span style="color:#FFF;font-size:14px;padding:0 0 0 5px;">'+tmpl.header + '</span></h4>' :
			'',
			// 确定按钮
			yesBtn = typeof tmpl.yesFn === 'function' ? 
				'<button style="background:#3079ED;" class="btn_highlight" id="easyDialogYesBtn">' + ( typeof tmpl.yesText === 'string' ? tmpl.yesText : '确定' ) + '</button>' :
				'',
			// 取消按钮	
			noBtn = typeof tmpl.noFn === 'function' || tmpl.noFn === true ? 
				'<button style="background:#3079ED;" class="btn_normal" id="easyDialogNoBtn">' + ( typeof tmpl.noText === 'string' ? tmpl.noText : '取消' ) + '</button>' :
				'',			
			// footer
			footer = yesBtn === '' && noBtn === '' ? '' :
			...
			dialogTmpl = [
			'<div class="easyDialog_content" id="easyDialog_content">',
				header,
...
//		}
		...
		//遮罩层对象
		var zhezhao = doc.getElementById( 'overlay' );
		// 定义弹出框的宽度
		dialogBox.style.width = options.width+'px';
		var edw = doc.getElementById( 'easyDialogWrapper' );
		edw.style.width = options.width+'px';
		
		eWidth = options.width;
//		eWidth = zhezhao.offsetWidth-options.width;
		// 定义弹出框的高度
		eDWrapper = doc.getElementById( 'easyDialog_content' );
		if( eDWrapper ){
			eDWrapper.style.height = options.height+'px';
		}
		
		eHeight = options.height;
//		eHeight = zhezhao.offsetHeight-options.height;
		...
		if( !options.follow ){	
			...
			var fh2 = '-';
			if (!docWidth) {
				docWidth = (zhezhao.offsetWidth>options.width?zhezhao.offsetWidth-options.width:0);
//				docWidth = (eWidth>dialogBox.offsetWidth?eWidth-dialogBox.offsetWidth:eWidth);
				fh = '+';
			}
			if (!docHeight) {
//				docHeight = eHeight;
				docHeight = (zhezhao.offsetHeight>options.height?zhezhao.offsetHeight-options.height:0);
				fh2 = '+';
			}
			dialogBox.style.marginLeft = fh + (widthOverflow ? docWidth/2 : eWidth/2) + 'px';
			dialogBox.style.marginTop = fh2 + (heigthOverflow ? docHeight/2 : eHeight/2) + 'px';			
		}
		else{
...
//		if (isIE) {
//			dialogBox.style.width = document.body.offsetWidth + 'px';
//		}
//		alert(dialogBox.style.width);
		// 防止select穿透固定宽度和高度
