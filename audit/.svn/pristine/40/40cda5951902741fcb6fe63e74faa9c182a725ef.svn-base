...
<head>
	...
	validateLogin();
		...
		function searchSingle(dateString){
			...
			var html="";
			html="<form id=\"formid\" action=\""+getRootPath()+"/CHART/itextUpload\" method=\"post\"><input type=\"hidden\" id=\"filename\" name=\"filename\" value='本科教学学生评教数据统计分析报告'></input><input type=\"hidden\" id=\"dateString\" name=\"dateString\" value='"+dateString+"'></input></form>";
			$("body").append(html);
			$("#formid").submit();
			$("#formid").remove();
		}
		...
		function searchSingleWord(dateString){
			var dateString = $(dateString).parent().parent().find("td").eq(2).text();
			var html="";
			html="<form id=\"formid\" action=\""+getRootPath()+"/CHART/poiWordUpload\" method=\"post\"><input type=\"hidden\" id=\"filename\" name=\"filename\" value='本科教学学生评教数据统计分析报告'></input><input type=\"hidden\" id=\"dateString\" name=\"dateString\" value='"+dateString+"'></input></form>";
			$("body").append(html);
			$("#formid").submit();
			$("#formid").remove();
		}
