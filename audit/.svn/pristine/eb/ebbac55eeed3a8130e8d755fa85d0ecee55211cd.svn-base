...
<head>
	...
	validateLogin();
		...
		function searchSingleWord(dateString){
			var dateString = $(dateString).parent().parent().find("td").eq(2).text();
			var html="";
			html="<form id=\"formid\" action=\""+getRootPath()+"/CHART/poiWordUpload\" method=\"post\"><input type=\"hidden\" id=\"filename\" name=\"filename\" value='审核评估状态数据分析报告'></input><input type=\"hidden\" id=\"dateString\" name=\"dateString\" value='"+dateString+"'></input></form>";
			$("body").append(html);
			$("#formid").submit();
			$("#formid").remove();
		}
