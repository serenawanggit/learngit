...
function clickObj(event, treeId, treeNode){
	var url = getRootPath() + "asEstimate/findAsEstimate.do?id="+treeNode.id;
	$.ajax({
		async : false,
		url:url,
		type:'get',
		...
		success:function(data){
			...
			var o = eval('(' + data +')');
			var asccurl = getRootPath() + "asCkeditorContent/findascc.do?eid=" + o.id;
			$.ajax({
				async:false,
				url:asccurl,
				dataType:"json",
...
function addjd() {
	...
	if (nodes.length == 0) {
		...
		str += "<span style='float:right; line-height:28px;font-size:12px;'>名称:&nbsp;&nbsp;</span>";
		str += "</div>";
		str += "<div style='width:70%;height:100%;float:left'>";
		str += "<input onclick='rightbox.window.clickjdname();return false';type='text' id='jdname' style='width:90%;height:28px;font-size:12px;'/>";
		str += "</div>";
		...
		str += "</div>";
		str += "</div>";
		str += "<div style='width:100%;height:20%;font-size:12px;padding-left:200px;padding-top:5px;' class='buttom_tips01'>";
		str += "<a onclick='rightbox.window.savejd("+nodes[0].id+","+nodes[0].level+");return false'; href = '#' style = 'text-decoration: none; margin-right:20px;text-align:center;height:30px; line-height:30px; color:#FFF;background:#2e8ae6;-webkit-border-radius: 3px;-moz-border-radius: 3px; border-radius: 3px;'>保存</a>";
		...
		str += "</div></td></tr>";
		$("#md", parent.document).css("top", "80px");
		$("#md", parent.document).css("left", "450px");
	}
	str += "</table>";
...
function savejd(id,level){
	...
	var str = "level="+level+"&parent="+id+"&name="+jdname;
	var url = getRootPath() + "asEstimate/saveAsEstimate.do";
		 $.ajax({
			 url:url,
			 async : false,
			 type : 'post',
...
function editjd() {
	...
	if (nodes.length == 0) {
		...
		str += "</div>";
		str += "<div style='width:70%;height:100%;float:left'>";
		str += "<input onclick='rightbox.window.clickjdname_u();return false'; value='"+nodes[0].name+"' type='text' id='jdname_u' style='width:90%;height:28px;font-size:12px;'/>";
		str += "</div>";
		str += "<div style='width:20%;height:100%;float:left'>";
		...
		str += "</div>";
		str += "<div style='width:100%;height:20%;font-size:12px;padding-left:200px;padding-top:5px;' class='buttom_tips01'>";
		str += "<a onclick='rightbox.window.updatejd("+nodes[0].id+","+nodes[0].level+","+nodes[0].pId+");return false'; href = '#' style = 'text-decoration: none; margin-right:20px;text-align:center;height:30px; line-height:30px; color:#FFF;background:#2e8ae6;-webkit-border-radius: 3px;-moz-border-radius: 3px; border-radius: 3px;'>保存</a>";
		str += "<a href = 'javascript:closeYS()' style = 'text-decoration: none; text-align:center;height:30px; line-height:30px; color:#FFF;background:#2e8ae6;-webkit-border-radius: 3px;-moz-border-radius: 3px; border-radius: 3px;'>取消</a>";
		str += "</div></td></tr>";
		$("#md", parent.document).css("top", "80px");
		$("#md", parent.document).css("left", "450px");
...
function updatejd(id,level,pId){
	...
	var str = "id="+id+"&level="+level+"&parent="+pId+"&name="+jdname_u;
	var url = getRootPath() + "asEstimate/updateAsEstimate.do";
		 $.ajax({
			 url:url,
			 async : false,
			 type : 'post',
...
function deletejd(){
	...
	if (nodes.length == 0) {
		...
		str += "<a onclick='rightbox.window.deletejd_action("+nodes[0].id+");return false'; href = '#' style = 'text-decoration: none; margin-right:20px;text-align:center;height:30px; line-height:30px; color:#FFF;background:#2e8ae6;-webkit-border-radius: 3px;-moz-border-radius: 3px; border-radius: 3px;'>确定</a>";
		str += "<a href = 'javascript:closeYS()' style = 'text-decoration: none; text-align:center;height:30px; line-height:30px; color:#FFF;background:#2e8ae6;-webkit-border-radius: 3px;-moz-border-radius: 3px; border-radius: 3px;'>取消</a>";
		str += "</div>";
		str += "</td></tr>";
		$("#md", parent.document).css("top", "80px");
...
function deletejd_action(id){
	var url = getRootPath() + "asEstimate/deleteAsEstimate.do?id=" + id;
