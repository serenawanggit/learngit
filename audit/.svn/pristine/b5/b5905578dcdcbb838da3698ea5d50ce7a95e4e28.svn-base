	...
	for ( var i = 0; i < datas.length; i++) {
		if(datas[i].id==a){
			...
			if(datas[i].id==1){
				...
				$("#entry").html("");
				var shtml = "<table style='width:100%'><tr><td style='color:#FFFFFF;font-size:60px;'><span style='color:#EB4E4E'>评估管理</span>系统</td></tr>" +
				"<tr><td style='color:#FFFFFF;font-size:20px;'>整改落实&nbsp;自评建设&nbsp;网上评估&nbsp;进场考查&nbsp;反馈提高<td></tr><tr><td><a href='javascript:void(0)' onclick='getContentTable(5,"+$('#zylxId').val()+")'><img src='images/assessmentcheck.png' style='padding-left:10%' /></a></td></tr></table>"
				$("#entry").html(shtml);
				showCheck(5);
...
function getContentTable(id,pgid){
	...
	if(id == 5){
		if(range == 9){
			if(pgid==1){
				window.location.href = sContentPath+"/page/SpecialtyManage/SpecialtyManageMain.jsp?zylxId="+pgid;
			}
			if(pgid==4){
				window.location.href = sContentPath+"/page/SpecialtyManage/SpecialtyManageMain.jsp?zylxId="+pgid;
			}
			...
			if(dataRange!=null&&dataRange.length==1){
				if(type!=null&&type!=""&&type!=undefined){
					if(type == 1){
						if(pgid==5){
							window.location.href = sContentPath+"/page/zenith/staticPage/Specialty.jsp?zylxId="+pgid;
						}else{
							window.location.href = sContentPath+"/page/zenith/staticPage/Specialty.jsp?zylxId="+pgid;
						}
					}else if(type == 2){
						if(pgid==5){
						  window.location.href = sContentPath+"/page/xueyuan/staticPage/Specialty.jsp?zylxId="+pgid;	
						}else{
							window.location.href = sContentPath+"/page/xueyuan/staticPage/Specialty.jsp?zylxId="+pgid;	
						}
						...
						if(pgid==5){
								window.location.href = sContentPath+"/page/wangping/staticPage/Specialty.jsp?zylxId="+pgid;
							}else{
								window.location.href = sContentPath+"/page/wangping/staticPage/Specialty.jsp?zylxId="+pgid;
							}
						
					}else if(type == 4){
					//	window.location.href = sContentPath+"/page/zenith/staticPage/siteVisits.jsp";
						if(pgid==5){
							window.location.href = sContentPath+"/page/wangping/staticPage/Specialty.jsp?zylxId="+pgid;
						}else{
							window.location.href = sContentPath+"/page/wangping/staticPage/Specialty.jsp?zylxId="+pgid;
