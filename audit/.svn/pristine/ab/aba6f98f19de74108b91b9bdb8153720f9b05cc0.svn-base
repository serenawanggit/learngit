...
public class MSOfficeGeneratorUtils {
	 ...
	 *   *           
	 * </ul>
	 *   * @param leftMargin   *            左边距   * @param rightMargin  
	 * *            右边距   * @param topMargin   *            上边距   * @param
	 * buttomMargin   *            下边距  
	 */
	...
	/**
	 *   * 打开word文档   *   * @param docPath   *            word文档路径   * @return
	 * 打开的文档对象  
	 */
	...
	/**
	 *   * 全局替换   *   * @param oldText   *            要替换的内容   * @param
	 * replaceObj   *            被替换的内容  
	 */
	public void replaceAll(String oldText, Object replaceObj) {
		...
		if (oldText.indexOf("image") != -1 || newText.indexOf(".bm;") != -1
				|| newText.lastIndexOf(".jpg") != -1
				|| newText.indexOf(".gif") != -1) {
			while (find(oldText)) {
				insertImage(newText);
				Dispatch.call(this.selection, "MoveRight");
			}
	 ...
	 * 
	 * @param tableIndex
	 *                       表格下标，从1开始   * @param fstCellRowIdx   *           
	 *            开始行   * @param fstCellColIdx   *            开始列   * @param
	 *            secCellRowIdx   *            结束行   * @param secCellColIdx  
	 *            *            结束列  
	 */
	 ...
	 * 
	 * @param numRows
	 *              *            拆分的行数，如果不想拆分行，请指定为1   * @param numColumns  
	 *            *            拆分的列数，如果不想拆分列，请指定为1  
	 */
	...
	public void insertToTable(List<String[]> list) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			for (int j = 0; j < strs.length; j++) {
				// 遍历表格中每一??单元格，遍历次数所要填入的?热菔?肯嗤?br /> Dispatch cell =
				// this.getCell(i + 1, j + 1);
				// 选中此单元格
				Dispatch.call(cell, "Select");
				// 写入?热莸酱说ピ?裰?br /> Dispatch.put(this.selection, "Text",
				// strs[j]);
				// 将插入点移动至下一??位置
			}
			this.moveDown(1);
	...
	/**
	 * 在当前插入点插入字符串,利用此方法插入一行text后，Word会默认选中它，如果再调用此方法，会将原来的内容覆盖掉，所以调用此方法后，
	 * 记得调用moveRight，将偏移量向右边移动一个位置 。
	 * 
	 * @param newText
	 *            要插入的新字符串
	 */
	...
	public Dispatch getTable(int tableIndex) {
		if (this.tables == null)
			this.getTables();
		if (tableIndex >= 0)
			this.table = Dispatch.call(this.tables, "Item",
	 ...
	 * 
	 * @param tableIndex
	 *                     表格索引   * @param cellRowIdx   *            行索引   * @param
	 *            cellColIdx   *            列索引   * @param txt   *            文本
	 *             
	 */
	 ...
	 *              *            另一个文档的磁盘路径   * @param tableIndex   *           
	 *            被拷贝的段落在另一格文档中的序号(从1开始)  
	 */
	public void copyParagraphFromAnotherDoc(String anotherDocPath,
			int paragraphIndex) {
		Dispatch wordContent = Dispatch.get(this.document, "Content")
				.toDispatch(); // 取得当前文档的内容
	 ...
	 * @param anotherDocPath
	 *                        另一个文档的磁盘路径   * @param tableIndex   *           
	 *            被拷贝的段落在另一格文档中的序号(从1开始)   * @param pos   *            当前文档指定的位置
	 *             
	 */
	public void copyParagraphFromAnotherDoc(String anotherDocPath,
	 ...
	 * @param anotherDocPath
	 *                        另一个文档的磁盘路径   * @param tableIndex   *           
	 *            被拷贝的表格在另一格文档中的序号(从1开始)   * @param pos   *            当前文档指定的位置
	 *             
	 */
	 ...
	 * 
	 * @param anotherDocPath
	 *                     另一个文档的磁盘路径   * @param shapeIndex   *           
	 *            被拷贝的图片在另一格文档中的位置   * @param pos   *            当前文档指定的位置  
	 */
	...
	/**
	 *   * 在指定的表格的指定行前面增加行   *   * @param tableIndex   *           
	 * word文件中的第N张表(从1开始)   * @param rowIndex   *            指定行的序号(从1开始)  
	 */
	...
	/**
	 *   * 在指定列前面增加表格的列   *   * @param tableIndex   *           
	 * word文档中的第N张表(从1开始)   * @param colIndex   *            指定列的序号 (从1开始)  
	 */
	...
	/**
	 *   * 根据索引获得当前表格的列对象   *   * @param columnIndex   *            列索引   * @return
	 * col  
	 */
	...
	/**
	 * 设置当前selection的字体   *   * @param fontName   *            字体名称，如“微软雅黑”   * @param
	 * isBold   *            是否粗体   * @param isItalic   *            是否斜体   * @param
	 * isUnderline   *            是否下划线   * @param rgbColor   *           
	 * 颜色值"1,1,1,1"   * @param Scale   *            字体间距   * @param fontSize  
	 * *            字体大小  
	 */
	@Deprecated
	 ...
	 * @param size
	 *            字体大小 12:小四  16:三号  * @param name 字体名称  例如：宋体，新宋体，楷体，隶书  
	 */
	...
	// //  file.delete();
	// //  File file = new
	// File("C:\DOCUME~1\ADMINI~1\LOCALS~1\Temp\test5411720146039914615.tmp");
	...
	//   Dispatch paragraphs = Dispatch.get(doc2, "Paragraphs").toDispatch();
	//   Dispatch paragraph = Dispatch.call(paragraphs, "Item", new
	...
	//   // officeUtils.moveEnd();
	//   // officeUtils.setFontScale("微软雅黑", false, false, false, "1,1,1,1",
	// 100,
	//   // 18);
