...
function showdata(data){
	manager = $("#datatable").ligerGrid({
		...
		columns : [ {
			...
			minWidth : 100,
			render: function(row){
				 return "<a  href='javascript:void(0)' style='margin-right:15px;border: none;cursor:pointer;' onclick=\"updatespecialty('"+row.ID+"','"+row.name+"','"+row.SPECIALTY_NAME+"','"+row.SPECIALTY_CODE+"','"+row.TYPE_NAME+"','"+row.IS_RZ+"','"+row.IS_ENGINEERING+"')\"><img src='/TQCC/page/roleDataShow/images/edit.png' title='修改' style='margin-top:5px;'/></a>"
...
function addspecialty() {
	var table="<table  style='margin: 0 auto;border-collapse:   separate;   border-spacing:   10px; '>" +
	  ...
	  "</select></td></tr>"+
	  "<tr style=''><td >专业名称：</td><td><input type='text' id='rolename'/></td></tr>" +
	  "<tr style=''><td >专业代码：</td><td><input type='text' id='remarkcontent'/></td></tr>"+
	  "<tr style='' id='tablewrap'><td>专业类别：<td><select id='test' name='sfgk'>";
