...
//	$("#asdsNav").html(str);
	$("#content").html(menuStr(3));
//	$("#tab-content2").html(menuStr(3));
...
function menuStr(value){
	...
	$.each(ary,function(i,obj){
		...
		if(obj.url != null){
			str += "onclick=iframeTab("+value+",'"+obj.url+"')";
		}
		str += " >"+obj.name+"</a>";
		// 二级菜单
		...
		if(obj.children != null && obj.children != undefined && obj.children.length > 0){
			...
			$.each(obj.children,function(j,obj2){
				if(obj2!=undefined){
					str += "<li><a href='javascript:void(0);'";
					if(obj2.url!=null){
						str += " onclick=iframeTab("+value+",'"+obj2.url+"')";
					}
				    str += ">"+ obj2.name +"</a>";
					...
					if(obj2.children != null && obj2.children != undefined && obj2.children.length > 0){
						...
						$.each(obj2.children,function(k,obj3){
							if(obj3!=undefined){
								str += "<li><a href='javascript:void(0);'";
								if(obj3.url != null){
									str += " onclick=iframeTab("+value+",'"+obj3.url+"')";
								}
								str += ">"+ obj3.name +"</a>";
							}
						});
...
function iframeTab(value,url){
	...
	if(url!="ASDS_table_main"){
		var path=getRootPath()+"/page/"+file+"/"+url+".jsp";
	}else{
		var path=getRootPath()+"/page/"+file+"/"+url+".html";
	}
...
function selectMuban(value){

	if(value=="2"){

		$("#content").html(menuStr(2));
		$("#tempIframe").attr("src",getRootPath()+"/page/ASDSReport/ASDS_tableG-2.jsp");
		$("#muban2").removeClass();
		$("#muban2").addClass("class1");
		$("#muban3").removeClass();
		$("#muban3").addClass("class2");
		$('#content').tendina('destroy');
		
	}else{

		$("#content").html(menuStr(3));
		$("#tempIframe").attr("src",getRootPath()+"/page/ASDSReport3.0/ASDS_tableG-2.jsp");
		$("#muban3").removeClass();
		$("#muban3").addClass("class1");
		$("#muban2").removeClass();
		$("#muban2").addClass("class2");
		$('#content').tendina('destroy');
