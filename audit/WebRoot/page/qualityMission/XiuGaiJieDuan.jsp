...
li.main-title-right{float:left;width:102%;height: 30px;margin:0px 0px 0px 0px;line-height:30px;vertical-align:middle;text-align: center;}
	...
	<div class="divClass"
		...
		<div  class="top_s_z">
			<div class="divClass" style="margin: 2% 0px 0px 2%; ">
				阶段任务名称： <input type="text" id="jdRwName" value="${jdRw.jdrwmc}" style="width: 68%;height: 80%;">
				<input type="hidden" id="zlRwId" value="${jdRw.zlrwid}"/>
				<input type="hidden" id="jdRwId" value="${jdRw.id}"/>
			</div>

			<div class="divClass" style="">
				<li style=" float:left;">任务开始时间： <input type="text" readonly="readonly" style="width:100px;height:24px;line-height:24px;vertical-align:middle;margin-left:5px;border:#DADADA 1px solid;padding-left:5px;" onclick="SelectDate(this)" id="jdRwStartTime" value="${jdRw.startTime}"></li>
			</div>

			<div  class="task_score" style=" ">
				<li style=" float:left;margin-left:2%;">任务范围 ：&nbsp;&nbsp;</li>
				<li style=" float:left;">
					<input type="radio" name="fw" id="fw2" onclick="getUserGroupList(2)" value="2" style="vertical-align:middle;">&nbsp;&nbsp;院系&nbsp;&nbsp; 
					<input type="radio" name="fw" id="fw3" onclick="getUserGroupList(3)" value="3" style="vertical-align:middle;">&nbsp;&nbsp;&nbsp;专业&nbsp;&nbsp;
				</li>
			</div>
		...
		<div  class="top_s_y">
			<ul>
				<li style=" float:left;width:100%; margin-top:8%;margin-left: 0em">
					任务截止时间：<input type="text" readonly="readonly" style="width:100px;height:24px;line-height:24px;vertical-align:middle;margin-left:5px;border:#DADADA 1px solid;padding-left:5px;" id="jdRwEndTime" onclick="SelectDate(this)" value="${jdRw.endTime}">
				</li>
				<li style=" float:left;width:610px; margin-top:2em;">人员：
					<select id="userGroup" onclick="getUserList()">
						<option value="">--请选择--</option>
					</select>
					<select id="userList" >
						<option value="">--请选择--</option>
					</select>
					<input type="submit" value="增加" onclick="addUser()" style="cursor:hand;color:#FFFFFF; border-width: 0px;width:50px;height:26px; background-color:#6DA770;">
					...
					<input type="text" id="names" style="height:24px;margin-left:0%;" value="${names}"readonly="readonly"></li>
			</ul>
		</div>
		<div class="divClass"
			style=" float:left; width:948px;/*  width:60%; */ height:60%; margin:0px 0px 0px 5%;border: 2px solid #dadada;">
			...
			<div class="divClass" style="  float:left; width:80px; height:26px;margin-left:40% ">
				<li >
					<input type="submit" onclick="updateJdRw()" value="保存" style="cursor:hand;color:#FFFFFF; border-width: 0px;width:50px;height:26px; background-color:#6DA770;"></a>
				<li>
			</div> 
		</div>
        ...
        <div class="divClass" style="height:50px;position: relative;right:0%;top:1%;">
				    <form id="queryForm" style="position: relative;left:1%;top:5%;" method="post" action="">
				        ...
				        <span class="ss_input11 w10 f_l" style="width: 80px;margin-top:0px;margin-left: 20px;">
				            <select  class="select_tips01" style="width: 80px;height: 22px;" id="checkType" onchange="getPage()">
				                <option selected="selected" value="name">指标名称</option>
				                <option value="code">指标代码</option>
				            </select>
				        </span>
				        <script></script>
				        <input id="checkValue" class="input_tips1 w10" type="text" style="width: 80px;" name="nameOrCodeVal"></input>
				          <a href="javascript:void(0);" onclick="getPage()" class="ss_btn01 f_l" href="javascript:onChangeSelect();" style="position:absolute;z-index:1;margin-left: 0px;margin-top: 0px;"></a>
				      </form>
...
$(document).ready(function () {
	...
	getUserGroupList(fw);
	if(fw == "1"){
		$("#fw1").attr("checked","checked");
	}else if(fw == "2"){
		$("#fw2").attr("checked","checked");
	}else if(fw == "3"){
		$("#fw3").attr("checked","checked");
	}
...
function getZbList(){
	...
	for(var i=0;i<zbs.length;i++){
		var record = zbs[i];
		zbIds.push(record.kgzbdm);
		var html ="";
		html += "<li id='"+record.kgzbdm+"'><input type='text' value='"+record.zbName+"'  style='width: 28%;margin-left:2%;margin-top:1%;'> ";
        html += "     <select name='sfs'>";
        if(record.sf == ">"){
        	html += "        <option value='>' selected='selected'>大于</option>";
        }else{
        	html += "        <option value='>'>大于</option>";
        }
        
        if(record.sf == "<"){
         	html += "        <option value='<' selected='selected'>小于</option>";
        }else{
        	html += "        <option value='<'>小于</option>";
        } 
        if(record.sf == "="){
        	html += "        <option value='=' selected='selected'>等于</option>";
        }else{
        	html += "        <option value='='>等于</option>";
        } 
        if(record.sf == "<="){
        	html += "        <option value='>=' selected='selected'>大于等于</option>";
        }else{
        	html += "        <option value='>='>大于等于</option>";
        } 
        if(record.sf == "<="){
        	html += "        <option value='<=' selected='selected'>小于等于</option></select>";
        }else{
        	html += "        <option value='<='>小于等于</option></select>";
        }
        html += "    <input type='text' name='values' value="+record.qwz+" style='width: 16%;'> ";
        html += "    <input type='hidden' name='ids' value='"+record.kgzbdm+"' style='width: 16%;'> ";
        html += "                <input type='button' onclick='deleteZb("+record.kgzbdm+")' value='删除' style='cursor:hand;color:#FFFFFF; border-width: 0px;width:50px;height:26px; background-color:#6DA770;'></li>";
		zbList.append(html);
	}
}
...
function updateJdRw(){
	...
	$.ajax({
		 url:"<%=basePath%>/zlrwController/updateJdRw.do",
